<section id="Service" class="Service  pb-4">
<div class="container">
	<div class="row mb-4 pt-4 pb-4">
		<div class="col">
		<h2></h2>
		</div>
	</div>
	<div class="row mb-4 pt-4 pb-4">
		<div class="col">
		<h2></h2>
		</div>
	</div>
	
	<div class="row mb-4 pb-3 text-center">
	
		<div class="col-md ">
			<img src="assets/img/social-media1png " width="90" class="rounded mb-1">
			<h4>Social Media</h4>
		</div>
		
		<div class="col-md">
			<img src="assets/img/1472004.svg" width="90" class="rounded mb-1">
			<h4>Content Management</h4>
		</div>
		
		<div class="col-md">
			<img src="assets/img/network.png" width="90" class="rounded mb-1">
			<h4>Social</h4>
		</div>
		
		<div class="col-md">
			<img src="assets/img/tourist.png" width="90" class="rounded mb-1">
			<h4>Tourism</h4>
		</div>
	</div>
	<div class="row mb-4 text-center">
	
		<div class="col-md">
			<img src="assets/img/edu.svg " width="90" class="rounded mb-1">
			<h4>Education</h4>
		</div>
		
		<div class="col-md">
			<img src="assets/img/exercise.png " width="90" class="rounded mb-1">
			<h4>Healthy</h4>
		</div>
		
		<div class="col-md">
			<img src="assets/img/innovation.png " width="90" class="rounded mb-1">
			<h4>Energy</h4>
		</div>
		
		<div class="col-md">
			<img src="assets/img/cloud.png" width="90" class="rounded mb-1">
			<h4>Smart City</h4>
		</div>
	</div>
</div>
</section>

<section id="Event" class="Event  pb-4">
<div class="container">

   <div class="d-flex bd-highlight mb-3">
  
  <div class="p-2 bd-highlight">
		<h3>Event</h3></div>

  <div class="ml-auto p-2 bd-highlight">
		<a href="" class="text-danger"><h3>Others...</h3></a></div>
   </div>
	
	<div class="row mb-4 text-center">
	
		<div class="col-md ">
			<img src="assets/img/warpedtour.jpg" width="250" class="rounded mb-1">
			<h4>throw away</h4>
		</div>
		
		<div class="col-md">
			<img src="assets/img/dwp.jpg " width="250" class="rounded mb-1">
			<h4>Set the road</h4>
		</div>
		
		<div class="col-md">
			<img src="assets/img/rockimring.jpg " width="250" class="rounded mb-1">
			<h4>War Service</h4>
		</div>
		
		<div class="col-md">
			<img src="assets/img/hellfest.jpg " width="250" class="rounded mb-1">
			<h4>Dest</h4>
		</div>
	</div>
</div>
</section>

<div class="container-fluid  text-light pb-1 pt-3 "style="background-color: #FCBA3F;">
<div class="container">
	
  <div class="d-flex bd-highlight mb-3">
  
  <div class="p-2 bd-highlight">
		<h3>Comunity</h3></div>

  <div class="ml-auto p-2 bd-highlight">
		<h3>66 komuNITAS</h3></div>
</div>
	
</div>
</div>

<section id="komunitas" class="komunitas pb-4">
<div class="container">
	<div class="d-flex bd-highlight mb-3">
  
  <div class="p-2 bd-highlight">
		<h3>Our Comunity</h3></div>

  <div class="ml-auto p-2 bd-highlight">
		<a href="" class="text-danger"><h3>Others...</h3></a></div>
   </div>
	
	<div class="row mb-4 text-center">
	
		<div class="col-md ">
			<img src="assets/img/hex.PNG " width="300" class="rounded mb-1">
			<h4>throw away</h4>
		</div>
		
		<div class="col-md">
			<img src="assets/img/educations.jpg " width="300" class="rounded mb-1">
			<h4>Set the road</h4>
		</div>
		
		<div class="col-md">
			<img src="assets/img/hexSocial.PNG " width="300" class="rounded mb-1">
			<h4>War Service</h4>
		</div>
	</div>
</div>
</section>

<div class="container-fluid text-light pb-1 pt-3 "style="background-color: #FCBA3F;">
<div class="container">
	
  <div class="d-flex bd-highlight mb-3">
  
  <div class="p-2 bd-highlight">
		<h3>Ours</h3></div>

  <div class="ml-auto p-2 bd-highlight">
		<h3>Pendampingan</h3></div>
</div>
	
</div>
</div>

<div class="container pt-5 pb-5">

<div class="d-flex flex-row bd-highlight mb-3">
  <div class="p-2 bd-highlight">
  <form>
  <button type="button" class="btn" style="background-color: #e0dfdc;">
  Traineer <span class="badge badge-light">4</span>
  </button>
  </form></div>
  <div class="p-2 bd-highlight">
  <button type="button" class="btn" style="background-color: #e0dfdc;">
  Mentor <span class="badge badge-light">4</span>
  </button></div>
  <div class="p-2 bd-highlight">
  <form>
  <button type="button" class="btn" style="background-color: #e0dfdc;">
  Company <span class="badge badge-light">4</span>
  </button>
  </form></div>
</div>
</div>

<section id="komunitas" class="komunitas pb-4">
<div class="container">
	<div class="d-flex bd-highlight mb-3">
  
  <div class="p-2 bd-highlight">
		<h3>Our Partner</h3></div>

  <div class="ml-auto p-2 bd-highlight">
		<a href="" class="text-danger"><h3>Others...</h3></a></div>
   </div>
	
<div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
<div class="container">
  <div class="carousel-inner">
    <div class="carousel-item active">
		<div class="row">
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner2.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner4.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner5.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		</div>
    </div>
    <div class="carousel-item">
      <div class="row">
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner6.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner7.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner8.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner9.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
    </div></div>
	
	
    <div class="carousel-item">
      <div class="row">
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner10.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner11.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner12.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner13.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
    </div></div>
	
	<div class="carousel-item">
      <div class="row">
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner14.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner15.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<a href=" "><img src="assets/img/partner16.png" class="d-block w-100" alt="..."></a>
			</div>
		</div>
		</div>
		
		<div class="col">
			<div class="card" style="width: 15rem;">
			<div class="card-body">
			<img src="assets/img/bg.png" class="d-block w-100" alt="...">
			</div>
		</div>
		</div>
		
    </div></div>

  <a class="carousel-control-prev" href="#carouselExampleControls" role="button" data-slide="prev">
    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
    <span class="sr-only">Previous</span>
  </a>
  <a class="carousel-control-next" href="#carouselExampleControls" role="button" data-slide="next">
    <span class="carousel-control-next-icon" aria-hidden="true"></span>
    <span class="sr-only">Next</span>
  </a>
</div>
</div>
</div>
</section>

<!--model regis-->
<div class="modal fade bd-example-modal-sm" id="regis" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Choose Yours</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      
  <div class="card text-center">
  <div class="card-header">
    <ul class="nav nav-tabs card-header-tabs">
      <li class="nav-item">
        <button type="button" class="btn btn-light active">Talent</button>
      </li>
      <li class="nav-item">
        <button type="button" class="btn btn-light" data-toggle="modal" data-target="#regismentor" data-dismiss="modal">Mentor</button>
      </li>
      <li class="nav-item">
        <button type="button" class="btn btn-light" data-toggle="modal" data-target="#regiscompany" data-dismiss="modal">Company</button>
      </li>
    </ul>
  </div>
  <div class="card-body">
  <img class="mb-4" src="assets/img/hexIcons.png" alt="" width="100">
  <form class="form-signin">
		
		<label for="inputEmail" class="sr-only">Email address</label>
		<input type="email" id="inputEmail" class="form-control mb-3" placeholder="Email address" required autofocus>
		
		<label for="inputName" class="sr-only">Username</label>
		<input type="name" id="inputName" class="form-control mb-3" placeholder="Username" required autofocus>
	
		  
		<label for="inputPassword" class="sr-only">Password</label>
		<input type="password" id="inputPassword" class="form-control mb-3" placeholder="Password" required>
	
  <button class="btn btn-primary btn-block" type="submit">Sign up</button>
  <p class="mt-3 mb-2 text-muted">Do you have an account ? </p>
  <button type="button" class="btn btn-link text-danger" data-toggle="modal" data-target="#login" data-dismiss="modal">
  <b>Login Now</b></button>
  </p>
  <p class="mt-2 mb-3 text-muted">&copy; 2017-2019</p>
  </form>
  </div>
</div>
	  
      <div class="modal-footer">
        
      </div>
    </div>
  </div>
</div>

<!--model mentor-->
<div class="modal fade bd-example-modal-sm" id="regismentor" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Choose Yours</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      
  <div class="card text-center">
  <div class="card-header">
    <ul class="nav nav-tabs card-header-tabs">
      <li class="nav-item">
        <button type="button" class="btn btn-light" data-toggle="modal" data-target="#regis" data-dismiss="modal">Talent</button>
      </li>
      <li class="nav-item">
        <button type="button" class="btn btn-light active">Mentor</button>
      </li>
      <li class="nav-item">
        <button type="button" class="btn btn-light" data-toggle="modal" data-target="#regiscompany" data-dismiss="modal">Company</button>
      </li>
    </ul>
  </div>
  <div class="card-body">
  <img class="mb-4" src="assets/img/hexIcons.png" alt="" width="100">
  <form class="form-signin">
		
		<label for="inputEmail" class="sr-only">Email address</label>
		<input type="email" id="inputEmail" class="form-control mb-3" placeholder="Email address" required autofocus>
		
		<label for="inputName" class="sr-only">Username</label>
		<input type="name" id="inputName" class="form-control mb-3" placeholder="Username" required autofocus>
		
		  
		<label for="inputPassword" class="sr-only">Password</label>
		<input type="password" id="inputPassword" class="form-control mb-3" placeholder="Password" required>
	
  <button class="btn btn-primary btn-block" type="submit">Sign up</button>
  <p class="mt-3 mb-2 text-muted">Do you have an account ? </p>
  <button type="button" class="btn btn-link text-danger" data-toggle="modal" data-target="#login" data-dismiss="modal">
  <b>Login Now</b></button>
  </p>
  <p class="mt-2 mb-3 text-muted">&copy; 2017-2019</p>
  </form>
  </div>
</div>
	  
      <div class="modal-footer">
       
      </div>
    </div>
  </div>
</div>

<!--model company-->
<div class="modal fade bd-example-modal-sm" id="regiscompany" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Choose Yours</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      
  <div class="card text-center">
  <div class="card-header">
    <ul class="nav nav-tabs card-header-tabs">
      <li class="nav-item">
        <button type="button" class="btn btn-light" data-toggle="modal" data-target="#regis" data-dismiss="modal">Telent</button>
      </li>
      <li class="nav-item">
        <button type="button" class="btn btn-light" data-toggle="modal" data-target="#regismentor" data-dismiss="modal">Mentor</button>
      </li>
      <li class="nav-item">
        <button type="button" class="btn btn-light active">Company</button>
      </li>
    </ul>
  </div>
  <div class="card-body">
  <img class="mb-4" src="assets/img/hexIcons.png" alt="" width="100">
  <form class="form-signin">
		
		<label for="inputEmail" class="sr-only">Company Email</label>
		<input type="email" id="inputEmail" class="form-control mb-3" placeholder="Company Email" required autofocus>
		
		<label for="inputName" class="sr-only">Company Name</label>
		<input type="name" id="inputName" class="form-control mb-3" placeholder="Company Name" required autofocus>
		
		  
		<label for="inputPassword" class="sr-only">Password</label>
		<input type="password" id="inputPassword" class="form-control mb-3" placeholder="Password" required>
	
  <button class="btn btn-primary btn-block" type="submit">Sign up</button>
  <p class="mt-3 mb-2 text-muted">Do you have an account ? </p>
  <button type="button" class="btn btn-link text-danger" data-toggle="modal" data-target="#login" data-dismiss="modal">
  <b>Login Now</b></button>
  </p>
  <p class="mt-2 mb-3 text-muted">&copy; 2017-2019</p>
  </form>
  </div>
</div>
	  
      <div class="modal-footer">
       
      </div>
    </div>
  </div>
</div>



<!--LOGIN-->
<div class="modal fade bd-example-modal-sm" id="login" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Please Sign In</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body text-center">
      <form class="form-signin" action="next.html" method="post">
		<img class="mb-4" src="assets/img/hexIcons.png" alt="" width="100">
		
		<label for="inputName" class="sr-only">Username</label>
		<input type="name" id="inputEmail" class="form-control mb-3" placeholder="Username" required autofocus>
		
			  
		<label for="inputPassword" class="sr-only">Password</label>
		<input type="password" id="inputPassword" class="form-control mb-3" placeholder="Password" required>
		
  <button class="btn btn-primary btn-block" type="submit">Sign up</button>
  
  <p class="mt-3 mb-2 text-muted">Don't have an account ? Sign Up 
  <button type="button" class="btn btn-link text-danger" data-toggle="modal" data-target="#regis" data-dismiss="modal">
  <b>Daftar Sekarang</b></button>
  </p>
  
  <p class="mt-2 mb-3 text-muted">&copy; 2017-2019</p>
  </form>
      </div>
      <div class="modal-footer">
      </div>
    </div>
  </div>
</div>
<div class="container-fluid text-light pb-1 pt-3 "style="background-color: #3a3a3a;">
<div class="container">
<h6>azzzz</h6>
</div>
</div>
<div class="container-fluid text-light pb-1 pt-3 "style="background-color: #717171;">
<div class="container">
	
	<div class="row">
		<div class="col-8">
		<img src="assets/img/hexlogo.png" width="190">
		</div>
	
	<div class="col-4">
	<form>
	<h6>Kirim Pesan :</h6>
	<div class="form-group">
    <input type="email" class="form-control" id="exampleFormControlInput1" placeholder="Nama Anda...">
	</div>
	
	<div class="form-group">
    <input type="email" class="form-control" id="exampleFormControlInput1" placeholder="Email...">
	</div>
	
	<div class="form-group">
    <textarea class="form-control" id="exampleFormControlTextarea1" placeholder="Pesan..." rows="4"></textarea>
	</div>
	<button type="submit" class="btn btn-primary">Kirim Pesan</button>
	</form>
	</div>
	</div>
	
<div class="d-flex flex-row bd-highlight mb-3">
  <div class="p-2 bd-highlight">
  <a href="https://www.youtube.com/" target="_blank" rel="nofollow"><img src="assets/img/yt.png" width="30" class="rounded-circle"></a>
  </div>
  <div class="p-2 bd-highlight">
   <a href="https://www.facebook.com/" target="_blank" rel="nofollow"><img src="assets/img/fb.png" width="30" class="rounded-circle"></a>
  </div>
  <div class="p-2 bd-highlight">
  <a href="https://www.instagram.com/?hl=id" target="_blank" rel="nofollow"><img src="assets/img/ig.png" width="30" class="rounded-circle"></a>
  </div>
  <div class="p-2 bd-highlight">
  <h6>Tentang</h6>
  </div>
  <div class="p-2 bd-highlight">
  <h6>Kontak</h6>
  </div>
</div>
</div>

</div>

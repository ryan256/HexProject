<section id="eventM" class="eventM pb-4">
	<div class="container">

		<div class="row justify-content-center mt-4">

			<div class="col-lg">

				<div class="text-center shadow bg-white rounded pb-1">
					<img src="<?php echo base_url(); ?>assets/img/rockimring.jpg" width="720" class="rounded" alt="...">
				</div>

				<div class="d-flex justify-content-start pt-3">
					<div class="p-2 bd-highlight"><button type="button" class="btn btn-success btn-block mr-5">
							Daftar
						</button></div>

					<div class="p-2 bd-highlight"><a href="<?php echo base_url(); ?>assets/img/rockimring.jpg" type=" button" class="btn btn-info btn-block mr-5" download>
							Unduh
						</a></div>
				</div>
			</div>

			<div class="col-lg-4">

				<div class="row no-gutters bg-light position-relative mb-3">
					<div class="col-md-6 mb-md-0 p-md-4">
						<img src="<?php echo base_url(); ?>assets/img/rockimring.jpg" class="w-100" alt="...">
					</div>
					<div class="col-md-6 position-static p-4 pl-md-0">
						<h5 class="mt-0">Event 3</h5>
						<a href="<?php echo base_url(); ?><?= $client['position']; ?>/event3" class="stretched-link text-decoration-none text-dark">Go somewhere</a>
					</div>
				</div>

				<div class="row no-gutters bg-light position-relative mt-3 mb-3">
					<div class="col-md-6 mb-md-0 p-md-4">
						<img src="<?php echo base_url(); ?>assets/img/hellfest.jpg" class="w-100" alt="...">
					</div>
					<div class="col-md-6 position-static p-4 pl-md-0">
						<h5 class="mt-0">Event 3</h5>
						<a href="<?php echo base_url(); ?><?= $client['position']; ?>/event4" class="stretched-link text-decoration-none text-dark">Go somewhere</a>
					</div>
				</div>

				<div class="row no-gutters bg-light position-relative mt-3 mb-3">
					<div class="col-md-6 mb-md-0 p-md-4">
						<img src="<?php echo base_url(); ?>assets/img/dwp.jpg" class="w-100" alt="...">
					</div>
					<div class="col-md-6 position-static p-4 pl-md-0">
						<h5 class="mt-0">Event 3</h5>
						<a href="<?php echo base_url(); ?><?= $client['position']; ?>/event2" class="stretched-link text-decoration-none text-dark">Go somewhere</a>
					</div>
				</div>

			</div>

		</div>

	</div>
	</div>
</section>